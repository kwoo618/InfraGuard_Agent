...
- **대회 기간(2026 우수작품경진대회) 추가 개발은 1인 개발(최강우)입니다.** 작성자가 PR 체크리스트와 테스트 결과를 확인한 뒤 직접 merge합니다.
  리뷰어(최소명)는 변경 내용을 공유하기 위해 지정합니다. 리뷰어의 승인을 merge 조건으로 두지 않습니다.
...
- 동시 가상 사용자 수(Locust `--users`, API 파라미터 `target_tps`)는 **30~50명**으로 설정합니다. 처리량(TPS) 목표가 아닙니다.
