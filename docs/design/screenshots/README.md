...
두 보기 모두 어두운 톤이다(토글할 때 밝기가 튀지 않게 배경·글자색을 맞췄다). 용어도 처음 보는 사람 기준으로 고친 뒤 다시 캡처했다.
...
| `a-endpoint-terms-005741.png` | 상세 | 요청 종류별 응답 시간(/light 가벼운 요청, /heavy 무거운 작업, /flaky 가끔 실패하는 요청, /health 상태 확인)과 서버별 처리한 요청 수 | 같음 |
