...
1. **동시 가상 사용자 수**(부하 세기, 최대 50)와 **테스트 시간(초)** 을 입력하고 **[진단 시작]** 클릭
2. 부하 테스트 → 메트릭 수집 → AI 병목 진단 로그가 실시간(SSE)으로 흐른다
3. 병목이 감지되면 **승인 모달**이 뜬다 → **[승인]** 시 실제 스케일링 실행
4. 스케일링 후 재검증 → **전/후 성능 비교 리포트**까지 확인

> **입력값 이해** — "동시 가상 사용자 수"는 처리량 목표가 아니라 Locust 동시 접속 수(`--users`)다. API 파라미터 이름은 `target_tps`지만 뜻은 같다. 결과로 나오는 TPS는 서버가 실제로 처리한 초당 요청 수(측정값)이며, 보통 입력값과 다르다. 병목을 제대로 관찰하려면 부하를 높이고(예: 40~50) 시간을 충분히(예: 20~30초) 준다.
...
동시 가상 사용자 수(`target_tps`, Locust `--users`, 처리량 목표 아님)와 테스트 기간(`duration`, 초)을
쿼리 파라미터로 받아 에이전트 루프를 시작하고, 진행 상황을 SSE로 실시간 스트리밍합니다.
다른 실행이 진행 중이거나 끊긴 실행의 부하 테스트가 남아 있으면 409를 반환합니다.

```json
// Request
GET /api/v1/agent/start?target_tps=40&duration=60
